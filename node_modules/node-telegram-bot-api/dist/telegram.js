...
export class TelegramBot extends EventEmitter {
    ...
    async editMessageMedia(media, form = {}) {
        ...
        if (typeof media.media === "string" && regexAttach.test(media.media)) {
            ...
            delete payload.fileOptions;
            const attachName = "0";
            const data = media.media.replace("attach://", "");
            const { file } = await prepareFile(data, media.fileOptions, this.options.filepath);
            if (!file)
                throw new FatalError(`Failed to process the replacement action for your ${media.type}`);
        ...
        const out = { ...form, media: stringify(media) };
    ...
    createNewStickerSet(userId, name, title, pngSticker, emojis, options = {}, fileOptions = {}) {
        const form = { ...options, user_id: userId, name, title, emojis };
        if (options.mask_position)
            form.mask_position = stringify(options.mask_position);
    ...
    addStickerToSet(userId, name, sticker, emojis, stickerType = "png_sticker", options = {}, fileOptions = {}) {
        ...
        const form = { ...options, user_id: userId, name, emojis };
        if (options.mask_position)
            form.mask_position = stringify(options.mask_position);
